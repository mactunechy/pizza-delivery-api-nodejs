...
handlers.checkout = (data,callback)=>{
	var acceptedMethods = ['post'];//add a get method when refactoring fot GUI
							...
							helpers.chargeCustomer(orderData,stripeToken,err=>{
								if(!err){
									...
									_data.update('orders',orderId,orderData,err=>{
										if(!err){
											callback(200,{'message':'Payment successul, you will recieve your receipt in you mail box'});
									//Sending thet receiot to the user via email
									//fetch users email 
									_data.read('users',token.userId,(err,userData)=>{
										if(!err %% userData){
											...
											callback(500)
											console.log('payment completed but failed to update the status of the order')
										}
									});
									}else{
									callback(500,{'Error':'Failed to make payment'});
								}
							});
						}else{
