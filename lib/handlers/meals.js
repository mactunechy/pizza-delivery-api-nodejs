					...
					isAdmin(tokenData.userId,userIsAdmin=>{
						if(userIsAdmin){
							//building the meal object
							 ...
							 var mealObject = {mealId,title,description,price}
							//finally lets  go ahead and save tehe meal to Disc
							_data.create('meals',mealId,mealObject,err=>{
								if(!err){
									callback(200);
								}else{
									callback(500,{'Error':'Failed to add the new meal to disc'});
									console.log('Failed to save new meal');
								}
							});
						}else{
							callback(403,{'Error':'Fordden, user is not authorised'});
						}
					});
				}else{
					...
					if(all){
						...
						_data.list('meals',(err,meals)=>{
							if(!err && meals){
								...
								callback(500,{'Error':'Failed to read meals list'});
							}
						})
					}
					if(id){
						...
						_data.read('meals',id,(err,mealData)=>{
							if(!err && mealData){
								...
								callback(404,{'Error':'Meal not found'});
							}
						})
					}
				}else{
...
if(id && token ){
	...
	_data.read('tokens',token,(err,tokenData)=>{
								...
								if(!err && mealData){
									...
									_data.update('meals',id,mealData,err=>{
										if(!err){
											...
											callback(500);
											console.log('failed to save meal changes');
										}
									});
								}else{
									callback(404,{'Error':'Meal not found'});
		...
		callback(400,{'Error':'No fields to update supplied'});
	}
